...
jQuery(function($){
      ...
      jQuery(document).on('click', 'input.select-img', function( event ){
        ...
        var AcademicProImage = wp.media.controller.Library.extend({
            defaults :  _.defaults({
        ...
        academic_pro_file_frame = wp.media.frames.academic_pro_file_frame = wp.media({
          button: {
            text: jQuery( this ).data( 'uploader_button_text' ),
          },
          state : 'tpiw-insert-image',
              states : [
                  new AcademicProImage()
              ],
          multiple: false  // Set to true to allow multiple files to be selected
        });

        // When an image is selected, run a callback.
        academic_pro_file_frame.on( 'select', function() {

          var state = academic_pro_file_frame.state('tpiw-insert-image');
